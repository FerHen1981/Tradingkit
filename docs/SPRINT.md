...
| **D-05** | wip | Middleware App | **Python fan-out wordt afgevoerd** (besluit Ferry 19-08). Nu D-02 naar .NET gaat en D-04 bevestigt dat notices via de .NET-receiver lopen, blijft er geen levend pad voor `main.py`/`router.py`/`brokers/**`/`risk.py`/`journal.py`. **Aanpak in twee stappen** zodat het reviewbaar blijft: (1) markeer die bestanden nu met een deprecation-header (`# DEAD PATH — replaced by mex-receiver (.NET), see D-05`) zodat niemand er per ongeluk aan hangt; (2) fysiek verwijderen na D-02 groen. Aparte commit per stap. **Start gemaakt 19-08** — headers geplaatst. |
