...
|---|---|---|---|
| **D-01** | wip | Middleware App | **`mex-viewer` serveert de hele vloot zonder authenticatie.** `viewer.py:161` faalt open: staat `VIEWER_PASSWORD` niet in de omgeving, dan zijn `/api/state`, `/api/command` en `/api/widget` publiek — accountnummers, saldi, buffers, open posities. **Direct:** variabele zetten + herstarten. **Structureel:** default omdraaien naar weigeren. _(was S-08 / SM-01)_ |
