...
| **D-40** | todo | Middleware App | **PMT account-blokkade wordt niet gerespecteerd door de receiver.** Als PMT's embed aangeeft dat een account geblokkeerd is (day cap of DLL getriggerd), stuurt de receiver het signaal tóch door naar PMT — ook al staat het TradingView-vinkje aan. De receiver moet vóór het forwarden de PMT-accountstatus controleren; geblokkeerde accounts mogen het signaal niet bereiken. Dit is het live handhavingspunt van D-02 (risk gate). Owner: .NET receiver (Middleware App). _(gevraagd door Ferry 20-08; volgt uit D-37-oplevering)_ |
...
| **D-28** | blocked | Middleware App + Legacy | **Notify-kanaal: zes uitbreidingen.** Status per uitbreiding + plan in `middleware/docs/D-28-NOTIFY-CHANNEL.md`. Live (uit routed_log-bewijs): (1) rich embeds ✅ (3) P&L + MFE/MAE bij close ✅. Half: (2) per-kanaal routing — de config-tabel en pure-lookup `notify_routing.py` staat, wacht op de .NET-dispatch die hem leest; (5) failure-severity tiering ✅, rate-limit ontbreekt; (6) Telegram-pariteit — routing klaar, endpoint aan .NET-kant ontbreekt. (4) chart-snapshot = aparte D-22, niet dubbelen. **Middleware App geleverd** (19-08): env-tabel + `webhook_for()`/`describe()` + 10 tests die borgen dat debug en routing in sync blijven. **Resteert voor Legacy:** 3 kleine hooks in `Mex.Journal.Receiver` — zie doc §*Wat er voor Legacy resteert*. |
