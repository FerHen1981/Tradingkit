...
| **7** | Qty-override (D-53) | `AccountQty` in `Program.cs:202` | er is een `MEX_ACCOUNT_QTY`-entry voor dit account | `multiple_accounts[0].quantity` wordt overschreven met het aantal contracten voor dat account (als string, zoals Pine hem stuurt); `body` wordt herbouwd en die versie gaat naar de forward | *(geen reject — deze gate stopt nooit)* | Ferry (env) + Middleware App |
...
**a. ✅ OPGELOST 18-09 — en andersom dan ik het eerst rechtzette.**
CLO had de qty-override op **#7** gezet, ná de blocked- en risk-gate. De code draaide
hem toen op regel 157, dus ervóór, en ik heb de tabel daarop aangepast naar #4.
Middleware App heeft vervolgens de **code** verplaatst naar ná de gates (D-53,
`Program.cs:202`) — precies om punt **b** hieronder op te lossen. **CLO's oorspronkelijke
volgorde was dus de goede; die van de code was het probleem.** De tabel staat weer op #7
en de verwijzingen naar de auto-DLL-gate zijn teruggeschoven naar #6.

**b. ✅ OPGELOST 18-09 (D-53).** De override herschreef `body` vóórdat de gates mochten
weigeren, waardoor `routed_*.jsonl` bij een reject een order-grootte droeg die nooit
verstuurd is. Middleware App heeft de override naar ná de gates verplaatst: reject-rijen
dragen nu de **onbewerkte Pine-body**, success-rijen dragen wat er echt over de draad ging.
...
✅ **OPGELOST 18-09 (D-53).** De gate overschrijft nu `multiple_accounts[0].quantity`
direct met het aantal contracten voor dat account — als string, zoals Pine hem stuurt —
en laat `quantity_multiplier` met rust. De env heet daarom nu **`MEX_ACCOUNT_QTY`**
(contracten per account); de oude naam wordt op start herkend en met een waarschuwing
als contract-counts gelezen. De onjuiste commentaarregel in `Program.cs` is weg.
