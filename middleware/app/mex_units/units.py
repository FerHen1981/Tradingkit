...
De middleware wordt vanuit `middleware/` gedraaid, dus we zetten de repo-root
even op `sys.path` zodat `backtest.config` te importeren is. Terugvallen op
een ingebakken kopie zou de registry stilzwijgend laten verouderen — dat mag
niet, dus faalt de import hieronder desnoods hard.
"""
...
# units.py → mex_units → app → middleware → repo-root
...
try:
    ...
    raise ImportError(
        ...
        "source voor tick-size en pointvalue (werkafspraken §3). Zorg dat de "
        f"repo-root ({_REPO_ROOT}) een backtest/config.py bevat; dit bestand "
        "houdt bewust geen eigen kopie van de specs."
    ) from exc
